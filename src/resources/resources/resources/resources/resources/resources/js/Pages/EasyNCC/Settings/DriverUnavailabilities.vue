...
const openModal = (item = null) => {
  if (item) {
    ...
    formStartDate.value = moment(item.start_date).format('YYYY-MM-DD');
    formEndDate.value = moment(item.end_date).format('YYYY-MM-DD');
    formStartTime.value = allDay ? '00:00' : moment(item.start_date).format('HH:mm');
    formEndTime.value = allDay ? '23:59' : moment(item.end_date).format('HH:mm');
  } else {
