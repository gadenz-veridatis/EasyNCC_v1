                            ...
                            <BRow>
                                ...
                                <BCol md="6" class="mb-3">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        class="form-control"
                                        :class="{ 'is-invalid': errors.passenger_count }"
                                        min="1"
                                    />
                                    <small v-if="errors.passenger_count" class="text-danger d-block mt-1">
                                        {{ errors.passenger_count[0] }}
                                    </small>
                                </BCol>

                                <!-- Price -->
                                <BCol md="6" class="mb-3">
                                    <label for="service_price" class="form-label">Prezzo</label>
                                    <input
                                        id="service_price"
                                        v-model.number="form.service_price"
                                        type="number"
                                        class="form-control"
                                        :class="{ 'is-invalid': errors.service_price }"
                                        step="0.01"
                                        min="0"
                                    />
                                    <small v-if="errors.service_price" class="text-danger d-block mt-1">
...
const form = ref({
    ...
    pickup_datetime: props.service?.pickup_datetime ? formatDateTimeForInput(props.service.pickup_datetime) : '',
    dropoff_datetime: props.service?.dropoff_datetime ? formatDateTimeForInput(props.service.dropoff_datetime) : '',
    passenger_count: props.service?.passenger_count || 1,
    service_price: props.service?.service_price || '',
...
const loadClients = async () => {
    try {
        const response = await axios.get('/api/users', {
            params: {
                role: 'committente',
                per_page: 500 // Load more for dropdowns
            }
        });
...
const submitForm = async () => {
    ...
    try {
        ...
        const submitData = {
            ...
            pickup_datetime: form.value.pickup_datetime ? form.value.pickup_datetime.replace('T', ' ') + ':00' : null,
            dropoff_datetime: form.value.dropoff_datetime ? form.value.dropoff_datetime.replace('T', ' ') + ':00' : null,
            // Add company_id from current user if creating new service
            company_id: isEdit.value ? props.service.company_id : (await getCurrentUser()).company_id
        };
