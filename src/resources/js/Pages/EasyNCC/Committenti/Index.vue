<template>
    ...
    <Layout>
        <PageHeader title="Committenti" pageTitle="EasyNCC" />

        <BRow>
            <BCol lg="12">
                <BCard no-body>
                    <BCardHeader class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Lista Committenti</h5>
                    ...
                    <BCardBody>
                        ...
                        <!-- Table -->
                        <div v-else-if="committenti.length > 0" class="table-responsive">
                            <table class="table table-hover table-bordered table-nowrap align-middle mb-0">
                                ...
                                <tbody>
                                    <tr v-for="committente in committenti" :key="committente.id">
                                        <td class="fw-medium">
                                            <span class="text-uppercase">{{ committente.surname || '-' }}</span>
                                            {{ committente.name || '' }}
                                        </td>
                                        <td>{{ committente.email || '-' }}</td>
                                        <td>{{ committente.phone || committente.client_profile?.phone || '-' }}</td>
                                        ...
                                        <td>
                                            ...
                                            <button
                                                ...
                                                @click="deleteCommittente(committente.id)"
                                                title="Elimina"
                                            >
                        ...
                        <div v-else class="text-center py-5">
                            ...
                            <h5 class="mt-3">Nessun committente trovato</h5>
                            <p class="text-muted">Non ci sono committenti che corrispondono ai criteri di ricerca.</p>
                        </div>
...
const deleteCommittente = async (id) => {
    const confirmed = await notify.confirm('Sei sicuro?', 'Vuoi eliminare questo committente?', { confirmText: 'Sì, elimina!' });

    if (confirmed) {
        try {
            await axios.delete(`/api/users/${id}`);
            notify.success('Il committente è stato eliminato.');
            loadCommittenti();
        } catch (err) {
            notify.error('Si è verificato un errore durante l\'eliminazione.');
            console.error('Error deleting committente:', err);
        }
    }
};
