<template>
    <Head title="Contabilità - Costi Fornitori" />

    <Layout>
        <PageHeader title="Costi Fornitori" pageTitle="Contabilità" />

        <BRow>
            <BCol lg="12">
                <BCard no-body>
                    <BCardHeader class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Sinottica Costi Fornitori</h5>
                    ...
                    <BCardBody>
                        ...
                        <div v-else class="table-responsive">
                            <table class="table table-bordered table-sm align-middle mb-0">
                                <thead>
                                    <tr>
                                        <th rowspan="2" class="text-start align-middle table-light" style="min-width: 220px;">Fornitore</th>
                                ...
                                <tbody>
                                    <template v-for="row in rows" :key="row.supplier_id">
                                        <tr class="cursor-pointer" :class="{ 'table-active': isExpanded(row.supplier_id) }"
                                            @click="toggleDetail(row.supplier_id)">
                                            <td class="fw-medium">
                                                <i :class="isExpanded(row.supplier_id) ? 'ri-arrow-down-s-fill' : 'ri-arrow-right-s-fill'"
                                        ...
                                        <tr v-if="isExpanded(row.supplier_id) && detailLoading[row.supplier_id]">
                                            <td :colspan="1 + columnDefinitions.length" class="text-center py-3 bg-light">
                                                <div class="spinner-border spinner-border-sm text-primary" role="status"></div>
                                                Caricamento dettaglio...
                                            </td>
                                        </tr>
                                        <template v-if="isExpanded(row.supplier_id) && !detailLoading[row.supplier_id] && detailData[row.supplier_id]">
                                            <tr v-if="detailData[row.supplier_id].length === 0">
                                                <td :colspan="1 + columnDefinitions.length" class="text-center text-muted py-2 bg-light">
                                                    Nessun servizio trovato
                                                </td>
                                            </tr>
                                            <tr v-for="svc in detailData[row.supplier_id]" :key="svc.service_id" class="bg-light">
                                                <td class="ps-4 small">
                                                    <div class="d-flex align-items-center gap-2">
...
export default {
    ...
    setup() {
        ...
        const costColumnKeys = new Set(['totale_costi', 'acconto', 'saldo']);
        const costColumns = computed(() => columnDefinitions.value.filter(c => costColumnKeys.has(c.key)));
        const paymentColumns = computed(() => columnDefinitions.value.filter(c => !costColumnKeys.has(c.key)));
        ...
        const toggleDetail = async (id) => {
            ...
            if (!detailData[id]) {
                ...
                try {
                    ...
                    const response = await axios.get(`/api/accounting-reports/supplier-costs/${id}`, { params });
        ...
        const loadData = async () => {
            ...
            try {
                const params = { month: selectedMonth.value };
                if (isSuperAdmin.value && selectedCompanyId.value) params.company_id = selectedCompanyId.value;
                const response = await axios.get('/api/accounting-reports/supplier-costs', { params });
        ...
        return {
            ...
            costColumns, paymentColumns, expandedItems, detailData, detailLoading,
...
.bg-cost { background-color: rgba(220, 53, 69, 0.15) !important; color: #842029; font-weight: 600; }
.bg-cost-light { background-color: rgba(220, 53, 69, 0.06) !important; }
