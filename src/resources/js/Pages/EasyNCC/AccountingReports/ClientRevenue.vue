<template>
    <Head title="Contabilità - Ricavi Committenti" />

    <Layout>
        <PageHeader title="Ricavi Committenti" pageTitle="Contabilità" />

        <BRow>
            <BCol lg="12">
                <BCard no-body>
                    <BCardHeader class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Sinottica Ricavi Committenti</h5>
                        ...
                        <div v-else class="table-responsive">
                            <table class="table table-bordered table-sm align-middle mb-0">
                                <thead>
                                    <!-- Group header row -->
                                    <tr>
                                        <th rowspan="2" class="text-start align-middle table-light" style="min-width: 220px;">Committente</th>
                                        <th :colspan="revenueColumns.length" class="text-center bg-revenue border-separator">
                                            RICAVI
                                        </th>
                                        <th :colspan="collectionColumns.length" class="text-center bg-collection">
                                            INCASSI
                                        </th>
                                    </tr>
                                    <!-- Column header row -->
                                    <tr>
                                        <th v-for="(col, idx) in revenueColumns" :key="col.key"
                                            class="text-end bg-revenue-light"
                                            :class="{ 'border-separator': idx === revenueColumns.length - 1 }"
                                            style="min-width: 130px;"
                                            :title="col.description">
                                            {{ col.label }}
                                            <sup class="text-muted">{{ getColumnIndex(col.key) }}</sup>
                                        </th>
                                        <th v-for="col in collectionColumns" :key="col.key"
                                            class="text-end bg-collection-light"
                                            style="min-width: 130px;"
                                            :title="col.description">
                                            {{ col.label }}
                                            <sup class="text-muted">{{ getColumnIndex(col.key) }}</sup>
                                ...
                                <tbody>
                                    <template v-for="row in rows" :key="row.client_id">
                                        <!-- Client summary row -->
                                        <tr class="cursor-pointer" :class="{ 'table-active': isExpanded(row.client_id) }"
                                            @click="toggleDetail(row.client_id)">
                                            <td class="fw-medium">
                                                <i :class="isExpanded(row.client_id) ? 'ri-arrow-down-s-fill' : 'ri-arrow-right-s-fill'"
                                        ...
                                        <tr v-if="isExpanded(row.client_id) && detailLoading[row.client_id]">
                                            <td :colspan="1 + columnDefinitions.length" class="text-center py-3 bg-light">
                                                <div class="spinner-border spinner-border-sm text-primary" role="status"></div>
                                                Caricamento dettaglio...
                                            </td>
                                        </tr>
                                        <template v-if="isExpanded(row.client_id) && !detailLoading[row.client_id] && detailData[row.client_id]">
                                            <tr v-if="detailData[row.client_id].length === 0">
                                                <td :colspan="1 + columnDefinitions.length" class="text-center text-muted py-2 bg-light">
                                                    Nessun servizio trovato
                                                </td>
                                            </tr>
                                            <tr v-for="svc in detailData[row.client_id]" :key="svc.service_id"
                                                class="bg-light">
                                                <td class="ps-4 small">
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span class="text-muted" style="font-size: 0.75rem;">
                                                                <span v-if="svc.pickup_address">{{ truncate(svc.pickup_address, 30) }}</span>
...
export default {
    ...
    setup() {
        ...
        const revenueColumnKeys = new Set(['acconto', 'saldo', 'totale_ricavi']);
        const revenueColumns = computed(() => columnDefinitions.value.filter(c => revenueColumnKeys.has(c.key)));
        const collectionColumns = computed(() => columnDefinitions.value.filter(c => !revenueColumnKeys.has(c.key)));

        const isExpanded = (clientId) => expandedClients.value.has(clientId);

        const toggleDetail = async (clientId) => {
            ...
            if (!detailData[clientId]) {
                ...
                try {
                    ...
                    const response = await axios.get(`/api/accounting-reports/client-revenue/${clientId}`, { params });
                    detailData[clientId] = response.data.services || [];
                } catch (error) {
                    console.error('Error loading client detail:', error);
                    detailData[clientId] = [];
                } finally {
                    detailLoading[clientId] = false;
                }
            }
        };
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const response = await axios.get('/api/accounting-reports/client-revenue', { params });
...
/* Thick border separating groups */
.border-separator {
