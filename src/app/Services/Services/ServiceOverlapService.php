...
class ServiceOverlapService
{
    ...
    public function checkOverlaps(array $serviceData, ?int $excludeServiceId = null): array
    {
        ...
        if ($vehicleId) {
            $vehicleUnavailConflicts = $this->checkVehicleUnavailabilityConflicts(
                $vehicleId,
                $effectiveStart,
                $effectiveEnd
            );
            $overlaps = array_merge($overlaps, $vehicleUnavailConflicts);
        }

        if (!empty($driverIds)) {
            $driverUnavailConflicts = $this->checkDriverUnavailabilityConflicts(
                $driverIds,
                $effectiveStart,
                $effectiveEnd
            );
            $overlaps = array_merge($overlaps, $driverUnavailConflicts);
        }
            ...
            foreach ($service->drivers as $serviceDriver) {
                if (in_array($serviceDriver->id, $checkDriverIds)) {
                    ...
                    $overlaps[] = [
                        ...
                        'driver_name' => $driverModel->display_name,
                        'service_departure' => $service->vehicle_departure_datetime,
                        'service_return' => $service->vehicle_return_datetime,
                    ];
                }
    ...
    /**
     * Check for vehicle unavailability conflicts.
     */
    protected function checkVehicleUnavailabilityConflicts(
        int $vehicleId,
        string $departure,
        string $return
    ): array {
        ...
        $unavailabilities = VehicleUnavailability::where('vehicle_id', $vehicleId)
            ->where('start_date', '<=', $return)
            ->where('end_date', '>=', $departure)
            ->with(['vehicle:id,license_plate,brand,model', 'unavailabilityType:id,name'])
            ->get();

        foreach ($unavailabilities as $unavail) {
            $conflicts[] = [
                'overlapping_service_id' => null,
                'overlapping_service_reference' => null,
                'overlap_type' => 'vehicle_unavailability',
                'vehicle_id' => $vehicleId,
                'vehicle_plate' => $unavail->vehicle->license_plate ?? null,
                'vehicle_brand' => $unavail->vehicle->brand ?? null,
                'vehicle_model' => $unavail->vehicle->model ?? null,
                'driver_id' => null,
                'driver_name' => null,
                'unavailability_id' => $unavail->id,
                'unavailability_reason' => $unavail->unavailabilityType->name ?? '',
                'unavailability_start' => $unavail->start_date->format('Y-m-d H:i'),
                'unavailability_end' => $unavail->end_date->format('Y-m-d H:i'),
            ];
        }
    ...
    /**
     * Check for driver unavailability conflicts.
     */
    protected function checkDriverUnavailabilityConflicts(
        array $driverIds,
        string $departure,
        string $return
    ): array {
        ...
        $unavailabilities = DriverUnavailability::whereIn('user_id', $driverIds)
            ->where('start_date', '<=', $return)
            ->where('end_date', '>=', $departure)
        ...
        foreach ($unavailabilities as $unavail) {
            $driverName = $unavail->user->display_name ?? trim(($unavail->user->name ?? '') . ' ' . ($unavail->user->surname ?? ''));
            $conflicts[] = [
                'overlapping_service_id' => null,
                'overlapping_service_reference' => null,
                'overlap_type' => 'driver_unavailability',
                'vehicle_id' => null,
                'vehicle_plate' => null,
                'vehicle_brand' => null,
                'vehicle_model' => null,
                'driver_id' => $unavail->user_id,
                'driver_name' => $driverName,
                'unavailability_id' => $unavail->id,
                'unavailability_reason' => $unavail->leaveType->name ?? '',
                'unavailability_start' => $unavail->start_date->format('Y-m-d H:i'),
                'unavailability_end' => $unavail->end_date->format('Y-m-d H:i'),
            ];
        }
    ...
    protected function mergeOverlaps(array $overlaps): array
    {
        ...
        foreach ($overlaps as $overlap) {
            // Unavailability conflicts are never merged with service overlaps
            if (in_array($overlap['overlap_type'], ['driver_unavailability', 'vehicle_unavailability'])) {
                $unavailabilityConflicts[] = $overlap;
                continue;
            }
        ...
        return array_merge(array_values($byService), $unavailabilityConflicts);
    }
    ...
    public function saveOverlaps(Service $service, array $overlaps, bool $confirmedByUser = true): void
    {
        ...
        // Insert new overlaps
        foreach ($overlaps as $overlap) {
            // Skip unavailability conflicts in service_overlaps table (they are checked dynamically)
            if (in_array($overlap['overlap_type'], ['driver_unavailability', 'vehicle_unavailability'])) {
                continue;
            }
