...
class UnavailabilityCalendarController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $driverUnavailabilities = DriverUnavailability::join('users', 'driver_unavailabilities.user_id', '=', 'users.id')
            ...
            ->map(function ($item) {
                return [
                    ...
                    'driver_name' => trim(($item->user->name ?? '') . ' ' . ($item->user->surname ?? '')),
        ...
        $vehicleUnavailabilities = VehicleUnavailability::join('vehicles', 'vehicle_unavailabilities.vehicle_id', '=', 'vehicles.id')
