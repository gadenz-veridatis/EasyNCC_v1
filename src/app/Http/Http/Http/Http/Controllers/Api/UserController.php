...
class UserController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('light')) {
            $query = User::select('id', 'name', 'surname', 'nickname', 'company_id', 'role')
                ->with('driverProfile:user_id,color,allow_overlapping');
            ...
            if ($request->boolean('is_committente')) {
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // Decode profile JSON string from FormData if needed
        if (isset($validated['profile']) && is_string($validated['profile'])) {
            $validated['profile'] = json_decode($validated['profile'], true);
        }
